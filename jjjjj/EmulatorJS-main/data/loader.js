(async function() {
    ...
    if (("undefined" != typeof EJS_DEBUG_XX && true === EJS_DEBUG_XX)) {
        ...
        // Allow loading a custom emulator file if the page provided one:
        // set window.EJS_customEmulator = "path/to/your/emulator.js"
        if (typeof window.EJS_customEmulator === "string" && window.EJS_customEmulator.trim() !== "") {
            try {
                await loadScript(window.EJS_customEmulator);
            } catch (e) {
                console.warn("Failed to load custom emulator, falling back to emulator.min.js", e);
                await loadScript("emulator.min.js");
            }
            // prefer minified css when using custom emulator by default, but allow override:
            if (typeof window.EJS_customEmulatorCSS === "string" && window.EJS_customEmulatorCSS.trim() !== "") {
                await loadStyle(window.EJS_customEmulatorCSS);
            } else {
